...
export class CodeGateAPI {
  ...
  static async getSystemStatus(): Promise<{ status: string; database: string; data_mode: string; version: string }> {
    const res = await fetch(`${API_BASE_URL}/system/status`, { signal: this.getSignal('system_status') });
    if (!res.ok) throw new Error('Failed to fetch system status');
    return res.json();
  }

  static async getOverview(): Promise<DashboardOverviewResponse> {
    const res = await fetch(`${API_BASE_URL}/dashboard/overview`, { signal: this.getSignal('overview') });
    if (!res.ok) throw new Error('Failed to fetch overview');
    return res.json();
  }

  static async getRepositories(): Promise<RepositoryDashboardItem[]> {
    const res = await fetch(`${API_BASE_URL}/dashboard/repositories`, { signal: this.getSignal('repositories') });
    if (!res.ok) throw new Error('Failed to fetch repositories');
    return res.json();
  }

  static async getRepositoryDetail(id: number): Promise<any> {
    const res = await fetch(`${API_BASE_URL}/dashboard/repositories/${id}`, { signal: this.getSignal(`repo_detail_${id}`) });
    if (!res.ok) {
  ...
  static async getPullRequests(): Promise<PRDashboardItem[]> {
    const res = await fetch(`${API_BASE_URL}/dashboard/pull-requests`, { signal: this.getSignal('pull_requests') });
    if (!res.ok) throw new Error('Failed to fetch PRs');
    return res.json();
  }

  static async getPullRequestDetail(id: number): Promise<PullRequestDashboardDetail> {
    const res = await fetch(`${API_BASE_URL}/dashboard/pull-requests/${id}`, { signal: this.getSignal(`pr_detail_${id}`) });
  ...
  static async getGitHubConnections(): Promise<{ 
    ...
    const res = await fetch(`${API_BASE_URL}/integrations/github/connections`, { signal: this.getSignal('github_connections') });
    if (!res.ok) throw new Error('Failed to fetch GitHub connections');
    return res.json();
  }

  static async installGitHubApp(): Promise<{ install_url: string }> {
    const res = await fetch(`${API_BASE_URL}/integrations/github/install`, { signal: this.getSignal('github_install') });
    if (!res.ok) {
  ...
  static async verifyGitHubConnection(id: number): Promise<any> {
    const res = await fetch(`${API_BASE_URL}/integrations/github/connections/${id}/verify`, { 
      method: 'POST',
  ...
  static async disconnectGitHubConnection(id: number): Promise<any> {
    const res = await fetch(`${API_BASE_URL}/integrations/github/connections/${id}/disconnect`, { 
      method: 'POST',
  ...
  static async syncGitHubConnection(id: number): Promise<{ discovered: number; created: number; updated: number; unchanged: number; removed_access: number; failed: number }> {
    const res = await fetch(`${API_BASE_URL}/integrations/github/connections/${id}/sync`, { 
      method: 'POST',
  ...
  static async retryAnalysis(analysisId: number): Promise<{ status: string }> {
    const res = await fetch(`${API_BASE_URL}/analyses/${analysisId}/retry`, { 
      method: 'POST',
